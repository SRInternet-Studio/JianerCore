        ...
        try:
            if endpoint == "send_message":
                data = self.send_message(
                    ...
                    receive_id=str(params.get("receive_id") or ""),
                    msg_type=str(params.get("msg_type") or "text"),
                    content=params.get("content") or {"text": ""},
                )
            elif endpoint == "reply_message":
                data = self.reply_message(
                    message_id=str(params.get("message_id") or ""),
                    msg_type=str(params.get("msg_type") or "text"),
                    content=params.get("content") or {"text": ""},
                )
            elif endpoint == "delete_message":
                data = self.delete_message(str(params.get("message_id") or ""))
            elif endpoint == "get_message":
                data = self.get_message(str(params.get("message_id") or ""))
            elif endpoint == "get_user":
                data = self.get_user(str(params.get("open_id") or params.get("user_id") or ""))
            elif endpoint == "get_chat":
                data = self.get_chat(str(params.get("chat_id") or ""))
            else:
                data = self._request(
                    str(params.get("method") or "POST"),
                    str(params.get("path") or endpoint),
                    params=params.get("params"),
                    json_body=params.get("json_body"),
                    data=params.get("data"),
                    files=params.get("files"),
                    auth=bool(params.get("auth", True)),
                )
            return {"status": "ok", "retcode": 0, "data": data}
        except Exception as exc:
            return {"status": "failed", "retcode": 1, "data": {"message": str(exc)}}
